...
def test_filter_single_npy():
    ...
    data_filter = load_data.filter_single_subset(data_origin, config.classes)
...
def test_filter_files():
    ...
    dataset = load_data.load_filter_files(fs, config.classes)
    print(dataset.shape)
    print(np.unique(dataset[:, -1]))
    data, ls = load_data.div_samples_labels(dataset)
    ls_1hot = load_data.convert_ls_1hot(ls, config.classes)
    print(Counter(ls))
