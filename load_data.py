...
class TrainValiTest:
    ...
    def load_final_test_samples(self, fname=cfg.final_test_f):
        final_test_set = np.load(fname)
        final_test_samples = final_test_set[:, :3000]
        if self.is_fft:
            final_test_samples = convert_to_freq_domain(final_test_samples)
            if self.fft_clip > 0:
                final_test_samples = final_test_samples[:, :self.fft_clip]
        ...
        if self.norm_flag == 1:
            ...
            final_test_samples = scale_normalization(final_test_samples, self.max_v, self.min_v)

        return final_test_samples
